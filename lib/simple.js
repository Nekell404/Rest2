...
conn.ws.on('CB:call', async (json) => {
    ...
    let pa7rick = await conn.sendTextWithMentions(kimd.from, `*${conn.user.name}* tidak bisa menerima panggilan ${kimd.isVideo ? `video` : `suara`}. Maaf kamu akan diblockir!!`)

    //conn.sendContact(kimd.from, global.pemilik, pa7rick)

    await sleep(5000)

    await conn.updateBlockStatus(kimd.from, "block")
...
//Template Button Image 2 ✓
  ...
  //Button Image ✓
  conn.sendButGif = async(id, text1, desc1, gam1, but = [], options1 = {}) => {
  let buttonMessage = {
  video: gam1,
  caption: text1,
  footer: desc1,
...
m.isBotAdmin = bot && bot.admin || false // Are you Admin?  
}
